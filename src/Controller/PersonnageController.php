...
class PersonnageController
{
	...
	public function vieillirAction(Request $request, Application $app)
	{
		...
		if ( $form->isValid() )
		{
			$personnages = $app['orm.em']->getRepository('\App\Entity\Personnage')->findAll();
			$token = $app['orm.em']->getRepository('\App\Entity\Token')->findOneByTag('VIEILLESSE');
			$ages = $app['orm.em']->getRepository('\App\Entity\Age')->findAll();
	...
	public function adminAddAction(Request $request, Application $app)
	{
		...
		{
			$participant = $app['orm.em']->getRepository('\App\Entity\Participant')->find($participant);
		}
	...
	public function adminTokenAddAction(Request $request, Application $app, Personnage $personnage)
	{
		...
		$token = $app['orm.em']->getRepository('\App\Entity\Token')->findOneByTag($token);
		...
		if ( $personnage->getPersonnageHasTokens()->count() % 2 == 0 )
		{
			if ( $personnage->getAge()->getId() != 5 )
			{
				$age = $app['orm.em']->getRepository('\App\Entity\Age')->findOneById($personnage->getAge()->getId() + 1);
	...
	public function adminUpdateDomaineAction(Request $request, Application $app)
	{
		...
		$domaines = $app['orm.em']->getRepository('App\Entity\Domaine')->findAll();
	...
	public function adminUpdateLangueAction(Request $request, Application $app)
	{
		...
		$langues = $app['orm.em']->getRepository('App\Entity\Langue')->findBy(array(), array('secret' => 'ASC', 'diffusion' => 'DESC', 'label' => 'ASC'));
	...
	public function adminUpdatePriereAction(Request $request, Application $app)
	{
        ...
        $repo = $app['orm.em']->getRepository('\App\Entity\Priere');
	...
	public function adminUpdateSortAction(Request $request, Application $app)
	{
        ...
        $repo = $app['orm.em']->getRepository('\App\Entity\Sort');
        $sorts = $repo->findList(
                $type,
                $value,
                ['by' =>  $order_by, 'dir' => $order_dir],
	...
	public function adminUpdatePotionAction(Request $request, Application $app)
	{
        ...
        $repo = $app['orm.em']->getRepository('\App\Entity\Potion');
	...
	public function adminUpdateIngredientAction(Request $request, Application $app, Personnage $personnage)
	{
		...
		if ( $form->isValid() )
		{
			...
			if ( $random && $random > 0 )
			{
				$ingredients = $app['orm.em']->getRepository('App\Entity\Ingredient')->findAllOrderedByLabel();
				shuffle( $ingredients );
	...
	public function adminUpdateRessourceAction(Request $request, Application $app, Personnage $personnage)
	{
		...
		if ( $form->isValid() )
		{
			...
			{
				$ressourceCommune = $app['orm.em']->getRepository('App\Entity\Ressource')->findCommun();
			...
			{
				$ressourceRare = $app['orm.em']->getRepository('App\Entity\Ressource')->findRare();
