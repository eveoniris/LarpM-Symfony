...
class ParticipantController
{
	...
	public function personnageNewAction(Request $request, Application $app, Participant $participant)
	{
		...
		$classes = $app['orm.em']->getRepository('\App\Entity\Classe')->findAllCreation();
		...
		$ages = $app['orm.em']->getRepository('App\Entity\Age')->findAllOnCreation();
	...
	{
		$regles = $app['orm.em']->getRepository('App\Entity\Rule')->findAll();
	...
	public function groupeJoinAction(Request $request, Application $app, Participant $participant)
	{
		...
		if ( $form->isValid() )
		{
				$data = $form->getData();
				$code = $data['code'];
				$groupeGn = $app['orm.em']->getRepository('\App\Entity\GroupeGn')->findOneByCode($code);
				if ( ! $groupeGn )
				{
	...
	{	
		$repo = $app['orm.em']->getRepository('\App\Entity\PersonnageSecondaire');
	...
	{
		$repo = $app['orm.em']->getRepository('\App\Entity\Religion');
	...
	{
		$repo = $app['orm.em']->getRepository('\App\Entity\Classe');
	...
	public function competenceAddAction(Request $request, Application $app, Participant $participant)
	{
		...
		if ( $form->isValid() )
		{
			...
			$competence = $app['orm.em']->find('\App\Entity\Competence', $competenceId);
