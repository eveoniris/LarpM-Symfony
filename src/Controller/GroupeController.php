...
{		
	...
	{
		$repo = $app['orm.em']->getRepository('\App\Entity\Groupe');
		$groupes = $repo->findAllOrderByNumero();
		$ressourceRares = new ArrayCollection($app['orm.em']->getRepository('\App\Entity\Ressource')->findRare());
		$ressourceCommunes = new ArrayCollection($app['orm.em']->getRepository('\App\Entity\Ressource')->findCommun());
	...
	public function adminIngredientAction(Request $request, Application $app, Groupe $groupe)
	{
		...
		if ( $form->isValid() )
		{
			...
			if ( $random && $random > 0 )
			{
				$ingredients = $app['orm.em']->getRepository('App\Entity\Ingredient')->findAll();
				shuffle( $ingredients );
	...
	public function restaurationAction(Request $request, Application $app)
	{
		...
		if ( $form->isValid() )
		{
			...
			{
				$j = $app['orm.em']->getRepository('\App\Entity\Participant')->find($joueur);
	...
	{
		$repo = $app['orm.em']->getRepository('\App\Entity\GroupeAllie');
		$alliances = $repo->findByAlliances();
		$demandeAlliances = $repo->findByDemandeAlliances();
		
		$repo = $app['orm.em']->getRepository('\App\Entity\GroupeEnemy');
	...
	public function adminParticipantAddAction(Request $request, Application $app)
	{
		...
		$repo = $app['orm.em']->getRepository('\App\Entity\Participant');
	...
	public function adminParticipantRemoveAction(Request $request, Application $app)
	{
		...
		$participant = $app['orm.em']->find('\App\Entity\Participant',$participantId);
	...
	public function placeAction(Request $request, Application $app)
	{
		...
		$groupe = $app['orm.em']->find('\App\Entity\Groupe', $id);
